...
def create_validate_code(size=(120, 30),
    ...
    strs = create_strs(draw, chars, length, font_type,
        ...
        if not isinstance(data, dict):
            ...
            if country or province or city:
                location = u'异常登录提醒：你上次登录ip为{ip} 地点在 {country} {province} {city}'.format(
                    ip=last_ip, country=country, province=province, city=city)
            else:
                location = u'异常登录提醒: 您上次登录ip为{ip} 地点无法确定'.format(
                    ip=last_ip)
...
def check_is_locked(ip, session):
    ...
    if r.get('USER-BLOCK-{ip}'.format(ip=ip)) or \
            r.get('USER-BLOCK-{session}'.format(session=session)):
        return True
    return False
